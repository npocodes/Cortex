...
local AnimMod = {}
AnimMod.__index = AnimMod

function AnimMod.Run(char: Model)
	print("Running Anim Script")
	local Character = char
	local Humanoid = Character:WaitForChild("Humanoid")
	if(not(char:IsDescendantOf(game.Workspace)))then
		repeat task.wait() until char:IsDescendantOf(game.Workspace)
	end
	...
	function onRunning(speed)
		...
		if speed > speedThreshold * heightScale then
			...
			setAnimationSpeed(math.min(speed, 16) / scale)
			pose = "Running"
		else
	...
	Humanoid:SetAttribute("Emote", "None")

	Humanoid:GetAttributeChangedSignal("Emote"):Connect(function()
		local msg = Humanoid:GetAttribute("Emote")
		...
		task.wait(5)
		Humanoid:SetAttribute("Emote", "None")--Reset
	end)
